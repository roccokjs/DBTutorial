...
class UsersDBHelper (context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun addAge(userid: String): Boolean {
        val db = writableDatabase
        val users = readUser(userid)
        ...
        users.forEach {
            values.put(DBContract.UserEntry.COLUMN_USER_ID, it.userid)
            values.put(DBContract.UserEntry.COLUMN_NAME, it.name)
            values.put(DBContract.UserEntry.COLUMN_AGE, it.age + 1)
        }
